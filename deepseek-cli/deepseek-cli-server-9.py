...
def _no_action_correction(attempt: int) -> tuple[str, str]:
    """
    Escalating correction text for the "announced an action but never
    executed it" retry, combined with assistant-turn prefill (see
    _TOOL_USE_PREFILL above). Wording alone wasn't reliable enough even
    with retries — this makes stalling syntactically hard, not just
    against the rules.
...
class ToolFilter:
    ...
    def call_with_filter(self, session_key: str, prompt: str) -> tuple[str, list]:
        ...
        while True:
            ...
            # ── Case 2: announced an action but never executed it ───────
            has_tool_use = any(b["type"] == "tool_use" for b in blocks)
            if (not has_tool_use and self._tools_offered
                    and looks_like_unexecuted_intent(raw_text)):
                if no_action_attempts >= MAX_NO_ACTION_RETRIES:
                    print(
                        "[tool_parse] response still announces action without a "
                ...
                print(
                    "[tool_parse] response announces an action with no tool_use block — "
                    f"forcing retry with prefill (attempt {no_action_attempts}/{MAX_NO_ACTION_RETRIES})",
                    flush=True,
                )
                suffix, prefill = _no_action_correction(no_action_attempts)
