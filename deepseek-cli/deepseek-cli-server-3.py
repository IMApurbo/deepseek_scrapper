...
def parse_response(text: str, valid_tools: set[str] | None = None) -> list:
    ...
    # Debug: log the full raw text so you can see what DeepSeek actually sent
    ...
    # ── Fallback: if no tool_use blocks found, try markdown fenced JSON ──────
    # DeepSeek sometimes emits: ```json\n{"name":"Bash","input":{...}}\n```
    # without any XML wrapper. Only attempt this if we got only text blocks.
    if not any(b["type"] == "tool_use" for b in blocks):
        # Use [^`] to avoid greedily swallowing multiple fenced blocks as one.
