...
def _parse_json_tool(raw: str) -> dict | None:
    ...
    try:
        ...
        pass
    # Attempt 2: fix trailing commas, then close unclosed braces/brackets.
    # IMPORTANT: naive brace counting is wrong when braces appear inside strings
    # (e.g. shell commands like "echo {a,b}"). We only attempt repair when the
    # imbalance is small (≤3) and the last non-whitespace char isn't a closer,
    # which strongly suggests genuine truncation rather than embedded braces.
    try:
        fixed = re.sub(r",\s*([}\]])", r"\1", cleaned)
        last_char = fixed.rstrip()[-1:] if fixed.rstrip() else ""
        opens     = fixed.count("{") - fixed.count("}")
        brackets  = fixed.count("[") - fixed.count("]")
        # Only repair if imbalance is small and text looks truncated
        if 0 < opens <= 3 or 0 < brackets <= 3:
...
def parse_response(text: str, valid_tools: set[str] | None = None) -> list:
    ...
    # ── Pre-pass 0: strip hallucinated conversation continuations ────────────
    ...
    if earliest < len(text):
        print(f"[pre-pass 0] stripping hallucinated continuation at pos {earliest}", flush=True)
        text = text[:earliest]
    ...
    if not any(b["type"] == "tool_use" for b in blocks):
        fence_pat = re.compile(r"```(?:\w+)?\s*(\{[^`]*?\})\s*```", re.DOTALL)
        rebuilt: list[dict] = []
        replaced = False
...
class ToolFilter:
    ...
    def call_with_filter(self, session_key: str, prompt: str) -> tuple[str, list]:
        ...
        for attempt in range(1, MAX_UNKNOWN_TOOL_RETRIES + 1):
            ...
            correction = (
                f"\n\nHuman: <tool_use_error>\n"
                f"You tried to call unknown tool(s): {', '.join(sorted(unknown))}.\n"
                f"Those tools do not exist. You MUST only call tools from this list:\n"
                f"{valid_list}\n"
                f"Please retry your last action using a valid tool from the list above.\n"
                f"</tool_use_error>\n\nAssistant:"
            )
            print(
