    ...
    def get_or_create(self, key: str) -> dict:
        with self._lock:
            if key not in self._sessions:
                ...
                print(f"[session] new ds_id={ds_id} key={key!r}")
            return self._sessions[key]

    def reset(self, key: str):
        """Force a fresh DeepSeek chat session for this key (e.g. on new Hermes conversation)."""
        with self._lock:
...
# All known tool names (exact casing) used to recognise native
...
CLAUDE_CODE_TOOL_NAMES = set(DEEPSEEK_TAG_TO_TOOL.values()) | {
    ...
    # Hermes Agent tools
    "terminal", "browser_navigate", "browser_snapshot", "browser_click",
    "browser_type", "browser_console", "browser_get_images", "browser_scroll",
    "browser_close", "web_search", "web_extract", "delegate_task",
...
class ToolFilter:
    ...
    A response is considered INVALID (and retried) when:
      - Claude Code sent tools but DeepSeek returned ONLY text (no tool_use blocks)
    ...
    TOOL_INTENT_SIGNALS = [
        ...
        "<bash>", "<python>", "<editor>",
        # Claude Code phrase signals
        "I'll run", "I will run", "Let me run",
        "I'll use the", "I will use the",
        "I'll call", "I will call",
        "Executing tool:", "Calling tool:",
        # Claude Code tool name mentions in action context
        "I'll use Bash", "I'll use Read", "I'll use Write", "I'll use Edit",
        "I'll use WebFetch", "I'll use WebSearch",
        "I'll use Agent", "I'll use TaskCreate",
        "using the Bash tool", "using the Read tool",
        "call the Bash", "call the Read", "call the Write",
        # Hermes Agent tool intent signals
        "I'll use terminal", "I'll use browser", "I'll use web_search",
        "I'll use execute_code", "I'll use delegate_task",
        "I'll navigate", "Let me navigate", "Let me search",
        "Let me check", "Let me execute",
        "<terminal>", "<browser_navigate>", "<web_search>", "<delegate_task>",
    ]
...
def messages():
    ...
    # Derive a stable session key from the conversation so each Hermes chat
    # thread gets its own DeepSeek session rather than sharing one global session
    # (which caused context loss between Hermes multi-turn exchanges).
    if msgs:
        first_user = next(
        ...
        if first_user:
            ...
            session_key = str(hash(str(raw)[:120]))
        else:
            session_key = "global"
    else:
        session_key = "global"
...
# ── Hermes / provider-detection stub routes ───────────────────────────────────
# Hermes Agent probes many endpoints to fingerprint the backend.
# Returning empty-but-valid JSON stops the 404 spam and lets Hermes
# fall through to the /v1/messages flow cleanly.
...
def api_tags():
    # Ollama-style model list — Hermes probes this
    return jsonify({
        "models": [
            {"name": "claude-sonnet-4-20250514", "model": "claude-sonnet-4-20250514"},
        ]
    })

@app.route("/api/show", methods=["POST"])
def api_show():
    return jsonify({"modelfile": "", "parameters": "", "template": ""})
