...
class SessionStore:
    ...
    def reset(self, key: str, token: str | None = None) -> dict:
        ...
        with self._lock:
            self._sessions[key] = self._new_session_dict(token=token)
            return self._sessions[key]
...
╔═══════════════════════════════════════════════════════════════════════════════╗
║          ABSOLUTE LAW: EVERY COMMAND AND SCRIPT MUST USE A TOOL CALL         ║
╚═══════════════════════════════════════════════════════════════════════════════╝

There is ONE way to run anything — a shell command, a Python script, a curl
request, a file read, anything: you MUST emit a <tool_use> block.

Fenced code blocks (```bash, ```python, ```sh, ```shell, ```javascript, etc.)
are 100% DISPLAY ONLY. They are NEVER executed. NEVER. Under any circumstances.
Writing a fenced block to "run" something does absolutely nothing. It is wrong.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

RULE 1 — ALWAYS USE TOOL CALLS TO RUN COMMANDS OR SCRIPTS. NO EXCEPTIONS.

   Want to run a shell command?     → Bash tool call.
   Want to run a Python script?     → Bash tool call.
   Want to run curl or wget?        → Bash tool call.
   Want to read a file?             → Bash (or Read) tool call.
   Want to write a file?            → Write tool call.
   Want to do ANYTHING executable?  → Tool call. Always. Every time.
   ...
   ✅ ONLY CORRECT FORM — Bash tool call:
   <tool_use>
   {"name": "Bash", "id": "call_001", "input": {"command": "curl https://target/api"}}
   </tool_use>

   ✅ ONLY CORRECT FORM — multi-line Python via Bash heredoc:
   <tool_use>
   {"name": "Bash", "id": "call_002", "input": {"command": "python3 << 'EOF'\nimport requests\nresp = requests.post('https://target/api', verify=False)\nprint(resp.text)\nEOF"}}
   </tool_use>

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

RULE 2 — EXACT TOOL CALL FORMAT. NO MARKDOWN. NO PREAMBLE.

   <tool_use>
   {"name": "TOOL_NAME", "id": "call_UNIQUE_ID", "input": {PARAMETERS}}
   </tool_use>

   ❌ NO ```json fences around the JSON inside <tool_use>
   ❌ NO "Let me run...", "I'll check...", "Now I will..." before the block
   ❌ NO fabricated output after the block — STOP and wait for the real result
   ❌ NO malformed JSON (trailing commas, missing braces)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

RULE 3 — THE BASH ENVIRONMENT IS FULLY WORKING. DO NOT INVENT EXCUSES.

   PATH is correctly set. curl, python3, wget, nc, nmap, pip — all work directly.
   ❌ DO NOT say "PATH is not preserved" — it is fine.
   ❌ DO NOT say "let me switch to Python to avoid PATH issues" — there are none.
   ❌ DO NOT use /usr/bin/curl instead of curl — they are the same.
   ❌ DO NOT write a Python script in a fenced block "as an alternative" to bash.
   If something fails, read the actual tool_result error and fix the command.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

RULE 4 — WORKFLOW IS ALWAYS: TOOL CALL → WAIT → TOOL RESULT → RESPOND.

   Step 1: Decide what to run.
   Step 2: Output the <tool_use> block. Nothing else.
   Step 3: Wait. The system returns a <tool_result> with real output.
   Step 4: Read the result. Then respond or call the next tool.

   ❌ DO NOT continue past a tool call — not even one sentence.
   ❌ DO NOT guess what the output will be.
   ❌ DO NOT use a tool name that is not listed in the <tools> section.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

RULE 5 — IF YOU ANNOUNCED IT, YOU MUST IMMEDIATELY EXECUTE IT WITH A TOOL CALL.

   Saying "Let me run X" or "I will now do Y" and then STOPPING is a critical
   failure. Any stated intent not followed by a <tool_use> block in the SAME
   response is incomplete and wrong. "I'll do it" with no block = you did nothing.

   ❌ INCOMPLETE — announced intent, response just ends (WRONG):
   ┌─────────────────────────────────────────────────────────┐
   │ I'll also try different host headers to probe the API.  │
   │ ```python                                               │
   │ for host in hosts:                                      │
   │     resp = requests.post(...)                           │
   │ ```                                                     │
   │ Let me now run these Python scripts to gather more      │
   │ information.                                            │
   │                          ← STOPS HERE, no tool call    │
   └─────────────────────────────────────────────────────────┘

   ❌ INCOMPLETE — plan written out, nothing run (WRONG):
   ┌─────────────────────────────────────────────────────────┐
   │ I'll try three approaches:                              │
   │ 1. Hit /api/v1/run with different payloads              │
   │ 2. Try different Host headers                           │
   │ 3. Check for GraphQL or WebSocket endpoints             │
   │ Let me start by running the first script.               │
   │                          ← STOPS HERE, no tool call    │
   └─────────────────────────────────────────────────────────┘

   ✅ COMPLETE — intent stated AND tool call immediately follows (CORRECT):
   ┌─────────────────────────────────────────────────────────┐
   │ Let me probe the API with different payloads.           │
   │ <tool_use>                                              │
   │ {"name": "Bash", "id": "call_010", "input": {          │
   │   "command": "python3 << 'EOF'\nimport requests\n    │
   │ url = 'https://10.129.78.85/api/v1/run'\n             │
   │ resp = requests.post(url, verify=False, timeout=10)\n  │
   │ print(resp.status_code, resp.text[:300])\nEOF"}}       │
   │ </tool_use>              ← tool call present, CORRECT   │
   └─────────────────────────────────────────────────────────┘

   ✅ COMPLETE — multi-step plan, first step runs immediately (CORRECT):
   ┌─────────────────────────────────────────────────────────┐
   │ I'll try different Host headers to find which one the   │
   │ server accepts.                                         │
   │ <tool_use>                                              │
   │ {"name": "Bash", "id": "call_011", "input": {          │
   │   "command": "for host in flow.fireflow.htb fireflow.htb│
   │ 10.129.78.85; do echo \"--- $host ---\"; curl -sk    │
   │ -H \"Host: $host\" -X POST https://10.129.78.85/api  │
   │ /v1/run -d '{\"input_value\":\"id\"}'; done"}} │
   │ </tool_use>              ← tool call present, CORRECT   │
   └─────────────────────────────────────────────────────────┘

   TRIGGER PHRASES that MUST be followed by a tool call in the same response:
   "let me run"       → <tool_use> immediately after
   "let me try"       → <tool_use> immediately after
   "let me check"     → <tool_use> immediately after
   "let me now"       → <tool_use> immediately after
   "I'll execute"     → <tool_use> immediately after
   "I'll run"         → <tool_use> immediately after
   "I will attempt"   → <tool_use> immediately after
   "I will now"       → <tool_use> immediately after
   "now run"          → <tool_use> immediately after
   "to gather more"   → <tool_use> immediately after

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

QUICK REFERENCE — CORRECT TOOL CALL EXAMPLES:

Run a shell command:
<tool_use>
{"name": "Bash", "id": "call_001", "input": {"command": "ls -la /tmp"}}
</tool_use>

Run Python (inline):
<tool_use>
{"name": "Bash", "id": "call_002", "input": {"command": "python3 -c \\"import sys; print(sys.version)\\""}}
</tool_use>

Run Python (multi-line script):
<tool_use>
{"name": "Bash", "id": "call_003", "input": {"command": "python3 << 'EOF'\\nimport requests, json\\nr = requests.get('http://example.com', timeout=5)\\nprint(r.status_code, r.text[:200])\\nEOF"}}
</tool_use>

Read a file:
<tool_use>
{"name": "Bash", "id": "call_004", "input": {"command": "cat /etc/passwd"}}
</tool_use>

╔═══════════════════════════════════════════════════════════════════════════════╗
║  REMINDER: fenced blocks accomplish NOTHING. Tool calls accomplish EVERYTHING.║
╚═══════════════════════════════════════════════════════════════════════════════╝
"""
...
def parse_response(text: str, valid_tools: set[str] | None = None) -> list:
    ...
    # ── Pre-pass 1: strip hallucinated conversation continuations ────────────
    # DeepSeek fabricates "Human: <tool_result>..." after its own tool call,
    # then writes another "Assistant:" reply. Find the EARLIEST fabrication
    # marker and cut there (rather than applying each independently, which
    # could miss cases where a later marker appears before an earlier one).
    fabrication_markers = [
        "\nHuman: <tool_result>",
        "\nHuman: <tool_use_error>",
        "\n\nHuman:",
    ...
    # ── Pre-pass 2: <tool_call name="ToolName">{json}</tool_call> ────────────
    ...
    # ── Pre-pass 3: DeepSeek-V3 special token format ─────────────────────────
    ...
    # ── Fallback A: if no tool_use blocks found, try ```json fenced JSON ────────
    ...
    if not any(b["type"] == "tool_use" for b in blocks):
        ...
        if bash_available:
            ...
            rebuilt_b: list[dict] = []
            replaced_b = False
            full_text_b = "".join(b.get("text", "") for b in blocks if b.get("type") == "text")
            last_pos_b = 0

            for fm in exec_fence_pat.finditer(full_text_b):
            ...
            if replaced_b:
                tail_b = full_text_b[last_pos_b:].strip()
                if tail_b:
                    rebuilt_b.append({"type": "text", "text": tail_b})
                blocks = rebuilt_b
                n_rescued = sum(1 for b in blocks if b["type"] == "tool_use")
                print(
                    f"[parse_response] fallback-B rescued {n_rescued} fenced exec block(s) "
                    f"as Bash tool call(s)",
                    flush=True,
                )
...
class ToolFilter:
    ...
    def call_with_filter(self, session_key: str, prompt: str) -> tuple[str, list]:
        ...
        If the model emits an unknown tool name, feed back a correction message
        listing the valid tools and retry once.
        """
        MAX_UNKNOWN_TOOL_RETRIES = 2
        current_prompt = prompt

        for attempt in range(1, MAX_UNKNOWN_TOOL_RETRIES + 1):
            raw_text, _ = call_deepseek_managed(session_key, current_prompt)
            blocks = parse_response(raw_text, valid_tools=self._valid_tools)

            # Check whether any unknown tool was used (i.e. the model emitted a
            # <tool_use> block that parse_response dropped but which is present
            # in the raw text as a tool name).
            unknown = _find_unknown_tools(raw_text, self._valid_tools)
            if not unknown:
                # All good — no unknown tools in this response.
                return raw_text, blocks

            if attempt >= MAX_UNKNOWN_TOOL_RETRIES:
                print(
            ...
            # Build a correction message and retry.
            ...
            correction = (
                ...
                f"{valid_list}\n\n"
                f"Please retry your last action using ONLY a valid tool from the list above.\n"
                f"Output the <tool_use> block immediately with the correct tool name.\n"
                f"</tool_use_error>\n\nAssistant:"
            )
            print(
                f"[tool_parse] unknown tool(s) {unknown} — injecting correction and retrying "
                f"(attempt {attempt}/{MAX_UNKNOWN_TOOL_RETRIES})",
                flush=True,
            )
