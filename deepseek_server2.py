...
# ─────────────────────────────────────────────────────────────
# Browser thread + scraper configuration
# ─────────────────────────────────────────────────────────────
...
# ── Playwright Browser Thread ─────────────────────────────────────────────────
# Playwright (via greenlets) must ALWAYS be called from the same OS thread it
# was started on.  We run a single "browser thread" that owns the scraper and
# processes all send_message calls via a queue.  Flask worker threads submit
# jobs and block on an Event until the result is ready.
# ─────────────────────────────────────────────────────────────────────────────

import queue as _queue
import traceback as _traceback

_browser_queue: _queue.Queue = _queue.Queue()   # jobs sent to browser thread
_SCRAPER_TIMEOUT  = 120   # seconds a caller will wait before giving up
_browser_thread_ref = None  # threading.Thread, set at startup


def _browser_thread_alive() -> bool:
    """True if the browser thread is running."""
    return _browser_thread_ref is not None and _browser_thread_ref.is_alive()


class _BrowserJob:
    """A single send_message call, submitted from a Flask thread."""
    def __init__(self, prompt: str):
        self.prompt  = prompt
        self.result  = None          # (response_md, reasoning_blocks, elapsed)
...
def _browser_thread_main():
    """
    Runs forever in its own OS thread.
    Owns the DeepSeekScraper — the ONLY place Playwright is ever called.
    """
    ...
    def _make_scraper_with_retry(retries: int = 3) -> "DeepSeekScraper | None":
        for attempt in range(1, retries + 1):
            try:
                s = _make_scraper()
                print(f"[browser-thread] Browser ready (attempt {attempt}).", flush=True)
                return s
            except Exception as e:
                print(f"[browser-thread] Start attempt {attempt}/{retries} failed: {e}", flush=True)
        print("[browser-thread] All restart attempts failed. Browser unavailable.", flush=True)
        return None

    print("[browser-thread] Starting …", flush=True)
    scraper = _make_scraper_with_retry()

    while True:
        job: _BrowserJob = _browser_queue.get()   # blocks until work arrives
        ...
        # If scraper failed to start / last restart failed, reject immediately
        if scraper is None:
            job.error = RuntimeError("Browser is not available — restart the server")
            job.done.set()
            continue

        try:
            job.result = scraper.send_message(job.prompt)
        except Exception as e:
            print(f"[browser-thread] Error: {e} — restarting browser …", flush=True)
...
def _send_via_browser_thread(prompt: str):
    """
    Submit a job to the browser thread and wait for the result.
    Raises TimeoutError or re-raises the browser thread exception.
    Safe to call from any Flask worker thread.
    """
    if not _browser_thread_alive():
        raise RuntimeError("Browser thread is not running — restart the server")
    job = _BrowserJob(prompt)
    _browser_queue.put(job)
    finished = job.done.wait(timeout=_SCRAPER_TIMEOUT)
    if not finished:
        raise TimeoutError(f"DeepSeek did not respond within {_SCRAPER_TIMEOUT}s")
    if job.error is not None:
        raise job.error
    return job.result


# Legacy shim — kept so old references give a clear error instead of NameError
def get_scraper():
    raise RuntimeError("Do not call get_scraper() directly — use _send_via_browser_thread()")
...
def messages():
    ...
    # ── Send to DeepSeek via dedicated browser thread ────────
    try:
        response_md, reasoning_blocks, _elapsed = _send_via_browser_thread(prompt)
    except Exception as e:
        print(f"\n[proxy] DeepSeek error: {e}", flush=True)
        _traceback.print_exc(file=sys.stdout)
        sys.stdout.flush()
        return jsonify({"error": {
            "type":    "api_error",
            "message": f"DeepSeek error: {e}",
        }}), 500
    if response_md.startswith("[Error]"):
        print(f"[proxy] DeepSeek returned error: {response_md}", flush=True)
        return jsonify({"error": {
            "type":    "api_error",
            "message": response_md,
        }}), 500
    ...
    print(f"[proxy] DeepSeek response: {len(response_md)} chars, {len(reasoning_blocks)} reasoning block(s)", flush=True)
    if not response_md.strip():
        print("[proxy] WARNING: DeepSeek returned empty response!", flush=True)
...
def count_tokens():
    ...
    # Estimate tokens for the full prompt (messages + system)
    prompt = _extract_prompt(body).strip()
    input_tokens = _estimate_tokens(prompt)

    # Also count any top-level system prompt tokens not yet folded in
    ...
    if system:
        input_tokens += _estimate_tokens(str(system))

    return jsonify({"input_tokens": max(input_tokens, 1)})
...
def health():
    return jsonify({
        ...
        "browser": "ready" if _browser_thread_alive() else "not started",
...
def _main():
    global _headless_flag, _search_flag, _deepthink_flag, _expert_flag
    global _browser_thread_ref
...
"""
    ...
    # Launch the dedicated browser thread (owns all Playwright calls)
    _browser_thread_ref = threading.Thread(
        target=_browser_thread_main, daemon=True, name="browser-thread"
    )
    _browser_thread_ref.start()

    if not args.no_warmup:
        print("[*] Waiting for browser to be ready ...")
        time.sleep(3)
        if not _browser_thread_ref.is_alive():
            print("[!] Browser thread died during startup — check errors above.")
        else:
            print("[+] Browser thread running. Proxy is live!\n")
